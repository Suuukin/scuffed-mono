...
# switch to float or integerthen update line
def operation_pressed(operator):
    operator = int(operator)
    State.operation = operator
    if float(txtDisplay.get()).is_integer():
        State.first_number = int(State.line)
    else:
        State.first_number = float(State.line)
    State.line = ''
    update_line(State.line)
...
def equals():
    # checks if second number is float or int
    if float(State.line).is_integer():
        State.second_number = int(State.line)
    else:
        State.second_number = float(State.line)
