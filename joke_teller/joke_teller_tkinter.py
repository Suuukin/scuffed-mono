...
# class to hold joke variable
class State:
    joke = None

# shuffles the joke_list and then outputs one joke then waits to output another
# this makes it so you can only get each joke once in each cycle
def joke_shuffle():
    while True:
        random.shuffle(joke_list)
        yield from joke_list


joke_generator = joke_shuffle()

# this gets an output from our shuffled jokes
# and then uses tkinter configure to update label with new text
def new_joke():
    joke = next(joke_generator)
    label.configure(text=joke)

# label that displays our joke
label = tk.Label(window, text=State.joke, font=myFont)
label.grid(column=0, row=0, columnspan=20)

# button that sets variable joke to new joke.
joke_button = tk.Button(window, text="Tell Me A Joke", font=myFont, command=new_joke)
joke_button.grid(column=0, row=1, columnspan=20, sticky="nsew")
